        ...
        if (plot_now == plot_steps):
            if plot_3d==False:
                for i in range(n_fibers):
                    ax.plot((all_x[i*Nb:(i+1)*Nb] - x_m),(all_z[i*Nb:(i+1)*Nb] - z_m),'-', color='gray')
            elif plot_3d==True:
                for i in range(n_fibers):
                    ax.plot3D((all_x[i*Nb:(i+1)*Nb] - x_m), \
                              (all_y[i*Nb:(i+1)*Nb] - y_m), \
                              (all_z[i*Nb:(i+1)*Nb] - z_m),color='gray', linestyle='-')
